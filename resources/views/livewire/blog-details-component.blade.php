        ...
        <div class="row">
            <div class="col-lg-8 col-md-12">
                ...
                <div class="comments-area">
                    ...
                    @endphp
                    @if ($reviews)
                    <h3 class="comments-title">{{ $reviews->count() }} Comments:</h3>
                    @endif
                                ...
                                <article class="comment-body">
                                    <footer class="comment-meta">
                                        <div class="comment-author vcard">
                                            <img src="{{ asset('assets/img/user.png') }}" class="avatar" alt="image">
                                            <b class="fn">{{ $review->name}}</b>
                                            <span class="says">says:</span>
                                        </div>

                                        <div class="comment-metadata">
                                            <a href="#">
                                                        <time>{{date('d F, Y', strtotime(   $review->created_at )) }}</time>
                                                    </a>
                                        </div>
                                    </footer>
                    ...
                    <div class="comment-respond">
                        ...
                        <form class="comment-form" wire:submit.prevent="addReview">
                            ...
                            <p class="comment-form-comment">
                                <label>Comment</label>
                                <textarea name="comment" id="comment" cols="45" rows="5" maxlength="65525" required="required"  wire:model="comment"></textarea>
                                @error('comment')
                            ...
                            <p class="comment-form-author">
                                ...
                                <input type="text" id="author" name="author" required="required" wire:model="name">
                                @error('name')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </p>
                            <p class="comment-form-email">
                                ...
                                <input type="email" id="email" name="email" required="required" wire:model="email">
                                @error('email')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </p>
